...
const EmeraldBg = () => (
  <>
    ...
    <div className="absolute -top-4 -right-4 w-16 h-16 bg-emerald-400/08 blur-xl rounded-full pointer-events-none" />
  </>
);
...
const phaseColorMap = {
  ...
  blue:    { badge: 'bg-blue-50 text-blue-700 border border-blue-200',    bar: 'bg-blue-500' },
  emerald: { badge: 'bg-emerald-50 text-emerald-700 border border-emerald-200', bar: 'bg-emerald-500' },
};
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative overflow-hidden tile-pop bg-white rounded-2xl p-5"
        ...
        {/* Film strip holes */}
        <div className="absolute top-1.5 left-0 right-0 flex gap-1.5 px-4 pointer-events-none opacity-[0.06]">
          {Array.from({ length: 16 }).map((_, i) => (
            <div key={i} className="flex-1 h-1.5 bg-emerald-500 rounded-[1px]" />
          ))}
        </div>
        {/* Camera lens rings */}
        <div className="absolute right-8 top-1/2 -translate-y-1/2 w-20 h-20 rounded-full border border-emerald-400/12 pointer-events-none" />
        <div className="absolute right-11 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full border border-emerald-400/15 pointer-events-none" />
        <div className="absolute right-14 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full border border-emerald-400/18 pointer-events-none" />
        <div className="relative z-10 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-zinc-900">Productions & Distribution</h1>
      ...
      {activeTab === 'Production Tracking' && (
        <div className="space-y-4">
          {activeProductions.map(p => {
            ...
            return (
              <div key={p.title} className="relative tile-pop bg-white rounded-2xl p-6 overflow-hidden"
                style={{ border: '1px solid rgba(16,185,129,0.12)', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                <EmeraldBg />
                <div className="relative z-10">
                  <div className="flex items-start justify-between mb-5">
                    <div>
                      <div className="text-base font-bold text-zinc-900">{p.title}</div>
                  ...
                  <div className="grid grid-cols-3 gap-6 mb-5">
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-xs font-mono font-semibold text-zinc-700">{p.budgetSpent}%</span>
                      </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-xs font-mono font-semibold text-emerald-600">{p.qualifiedSpend}%</span>
                      </div>
                      <ProgressBar pct={p.qualifiedSpend} color="bg-emerald-500" />
                    </div>
                    <div>
                      <div className="text-xs text-zinc-500 mb-1">Qualified Amount</div>
                      <div className="text-xs font-mono font-semibold text-emerald-600">{p.qualifiedAmount}</div>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-zinc-50 rounded-xl p-3 border border-zinc-200 border-l-2 border-l-emerald-400">
                      <div className="flex items-center gap-1.5 mb-1">
                        <CheckCircle2 size={12} className="text-emerald-500" />
                        <span className="text-xs text-zinc-600 font-medium">Current Milestone</span>
                      </div>
                      <div className="text-xs text-zinc-700">{p.milestone}</div>
                    </div>
                    <div className="bg-zinc-50 rounded-xl p-3 border border-zinc-200 border-l-2 border-l-blue-400">
                      <div className="flex items-center gap-1.5 mb-1">
                        <BarChart2 size={12} className="text-blue-500" />
                        <span className="text-xs text-zinc-600 font-medium">Cash Flow</span>
                      </div>
                      <div className="text-xs text-zinc-700">{p.cashFlow}</div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {activeProductions.length === 0 && (
            <div className="relative tile-pop bg-white rounded-2xl p-8 text-center overflow-hidden"
              style={{ border: '1px solid rgba(16,185,129,0.12)', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
              <EmeraldBg />
      ...
      {activeTab === 'Distribution' && (
        <div className="space-y-4">
            ...
            <div key={f.title} className="relative tile-pop bg-white rounded-2xl p-6 overflow-hidden"
              ...
              <div className="relative z-10">
                <div className="flex items-start gap-4 mb-5">
                  <div className="w-12 h-14 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-1.5 mt-2 flex-wrap">
                      {f.platforms.map(p => (
                        <span key={p} className="text-xs bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-full">{p}</span>
                      ))}
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="text-2xl font-bold font-mono text-zinc-900">{f.streams}</div>
                    <div className="text-xs text-emerald-600 font-medium">{f.streamGrowth} MoM</div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-zinc-50 rounded-xl p-3 border border-zinc-200 border-l-2 border-l-emerald-400">
                    ...
                    <div className="text-xs text-zinc-700">{f.festivalNote}</div>
                  </div>
                  <div className="bg-zinc-50 rounded-xl p-3 border border-zinc-200 border-l-2 border-l-zinc-300">
                    <div className="text-xs text-zinc-500 mb-1">Press</div>
                    <div className="text-xs text-zinc-700 italic">{f.pressNote}</div>
                  </div>
                  <div className="bg-zinc-50 rounded-xl p-3 border border-zinc-200 border-l-2 border-l-emerald-500">
                    <div className="text-xs text-zinc-500 mb-1">Incentive Status</div>
                    <div className="text-xs text-emerald-600 font-medium">{f.taxStatus}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="relative tile-pop bg-white rounded-2xl p-5 overflow-hidden"
            style={{ border: '1px solid rgba(16,185,129,0.12)', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            <EmeraldBg />
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-1 h-4 bg-emerald-500 rounded-full" />
                <div className="text-sm font-semibold text-emerald-700">Distribution Playbook</div>
              </div>
              <ul className="space-y-2 text-sm text-zinc-600">
                <li className="flex gap-2"><span className="text-emerald-500 flex-shrink-0">→</span> AI-driven audience-segment analysis to match each film to the right markets, festivals, and platforms</li>
                <li className="flex gap-2"><span className="text-emerald-500 flex-shrink-0">→</span> Horror/thriller → Hulu, Shudder, festival circuit (Blood in the Snow, Fantasia, SXSW)</li>
                <li className="flex gap-2"><span className="text-emerald-500 flex-shrink-0">→</span> Influencer micro-targeting (horror/indie film communities) to drive streaming lift</li>
                <li className="flex gap-2"><span className="text-emerald-500 flex-shrink-0">→</span> Cross-vertical leverage: film composers (Luke) and artists (Talise) create natural cross-promotional reach</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      {/* ── Activity ─────────────────────────────────────────────────────────── */}
      {activeTab === 'Activity' && (
        <div className="relative tile-pop bg-white rounded-2xl overflow-hidden"
          style={{ border: '1px solid rgba(16,185,129,0.12)', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <EmeraldBg />
          <div className="relative z-10">
            <div className="px-5 py-4 border-b border-zinc-100 bg-gradient-to-r from-emerald-50 to-transparent">
